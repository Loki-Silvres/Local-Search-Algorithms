...
class Map:
    ...
    def gaussian(self, x, mu = 0, sig = 1) -> float:
        return (
    ...
    def get_height(self, point : float) -> float:
        return np.sum([self.gaussian(point, mu = mu, sig = sig) for i, (mu, sig) in enumerate(zip(MEANS, SIGMAS))], axis= 0)

    def metric(self, point : float) -> float:
        return self.get_height(point)
        
    def fill_grid(self) -> None:

        cols_on_surface = np.linspace(MAP_START, MAP_END, WINDOW_WIDTH)
        rows_on_surface = self.get_height(cols_on_surface)
